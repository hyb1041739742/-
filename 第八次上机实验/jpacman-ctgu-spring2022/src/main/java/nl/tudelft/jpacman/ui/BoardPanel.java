    ...
    private void render(Square square, Graphics graphics, int x, int y, int width, int height) {
        ...
        for (Unit unit : square.getOccupants()) {
            if(unit instanceof Pellet ){
                if(square.getOccupants().size()>1)
                {
                    ((Pellet) unit).setVisibility(false);  //当有其他unit 和 pellet 共存的时候，将他设置成看不见
                    continue; //并且不进行画图
                }
                else if(square.getOccupants().size() == 1)
                {
                    ((Pellet) unit).setVisibility(true);
                }
            }
